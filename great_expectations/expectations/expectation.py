...
from ..exceptions.expectation_engine import UnimplementedBackendError, UnrecognizedDataAssetError
...
def lookup_backend(data):
    ...
    raise UnrecognizedDataAssetError("Unable to determine the expectation engine for data.")
...
class Expectation(ABC, metaclass=MetaExpectation):
    ...
    def __check_validation_kwargs_definition(self):
        ...
        for parent in self.mro():
            assert validation_kwargs_set <= set(getattr(parent, "validation_kwargs", set())), "Invalid Expectation " \
                                                                                           "definition for : " + \
                                                                                           self.__class__.__name__
        return True

    def build_validation_kwargs(self, configuration: Optional[ExpectationConfiguration]):
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]):
        if configuration is None:
            configuration = self.configuration
        try:
            assert configuration.expectation_type == self.expectation_type, "expectation configuration type does not match " \
                                                                "expectation type"
        except AssertionError as e:
            raise InvalidExpectationConfigurationError(str(e))
        return True

    def validate(self, data, configuration: Optional[ExpectationConfiguration] = None, runtime_configuration=None):
        if configuration is None:
            configuration = self.configuration
        result = self._validate(data, configuration=configuration, runtime_configuration=runtime_configuration)
        if isinstance(result, dict):
            result = ExpectationValidationResult(**result)
        return result

    def _validate(self, data, configuration: ExpectationConfiguration, runtime_configuration=None):
...
class DatasetExpectation(Expectation, ABC):
    ...
    def get_value_set_parser(backend_type: DatasetBackendTypes):
        ...
        raise GreatExpectationsError("No parser found for backend: " + str(backend_type))
    ...
    def parse_value_set(self, backend_type: DatasetBackendTypes, value_set: Union[list, set]):
...
class ColumnMapDatasetExpectation(DatasetExpectation, ABC):
    ...
    def get_validator(self, data: 'Dataset'):
        ...
        raise UnimplementedBackendError("Unable to find validator for the data type provided.")


    def _validate(self, data: 'Dataset', configuration: ExpectationConfiguration, runtime_configuration=None):
        validator = self.get_validator(data)
        result = validator(data, configuration, runtime_configuration)
        return result

    def _validate_pandas(self, data: 'Dataset', configuration: ExpectationConfiguration, runtime_configuration=None):
        import numpy as np

        validation_kwargs = self.build_validation_kwargs(configuration)
        runtime_configuration = configuration.build_runtime_configuration(runtime_configuration)
        result_format = runtime_configuration.result_format

        column = validation_kwargs.pop("column")
        series = data[column]
        element_count = int(len(series))
        if configuration.expectation_type in ['expect_column_values_to_not_be_null', 'expect_column_values_to_be_null']:
        ...
        boolean_mapped_success_values = self._validate_pandas_series(
            validated_values, **validation_kwargs, runtime_configuration=runtime_configuration)
        success_count = np.count_nonzero(boolean_mapped_success_values)

        unexpected_list = list(
            validated_values[boolean_mapped_success_values == False])
        ...
        return_obj = self.format_map_output(
            ...
            element_count, validated_values_count,
            len(unexpected_list),
