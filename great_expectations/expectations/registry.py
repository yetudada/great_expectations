...
def register_expectation(expectation: Type['Expectation']) -> None:
    ...
    if expectation_type in _registered_expectations:
        if _registered_expectations[expectation_type] == expectation:
            logger.info("Multiple declarations of expectation " + expectation_type + " found.")
            return
        else:
            logger.warning("Overwriting declaration of expectation " + expectation_type + ".")
...
def list_registered_expectation_implementations(expectation_root: Type['Expectation']) -> List[str]:
    registered_expectation_implementations = []
    for expectation_name, expectation_implementation in _registered_expectations.items():
