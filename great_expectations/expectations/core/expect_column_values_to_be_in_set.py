...
from ..expectation import ColumnMapDatasetExpectation, InvalidExpectationConfigurationError
...
class ExpectColumnValuesToBeInSet(ColumnMapDatasetExpectation):
    ...
    def validate_configuration(self, configuration: Optional[ExpectationConfiguration]):
        ...
        try:
            ...
            assert isinstance(configuration.kwargs["value_set"], (list, set)), "value_set must be a list or a set"
    ...
    def _validate_pandas_series(self, series: pd.Series, value_set: Union[list, set], **kwargs):
        ...
        if pd.api.types.is_datetime64_any_dtype(series):
            parsed_value_set = self.parse_value_set(backend_type=DatasetBackendTypes.PandasDataFrame,
                                                    value_set=value_set)
        else:
